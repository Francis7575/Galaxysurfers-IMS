...
const InventoryData = ({ activeFilter }: InventoryDataProps) => {
  ...
  const handleDelete = async (idinventory: number) => {
    ...
    try {
      const response = await fetch(
        ...
        }/inventory/delete-inventory/${idinventory}`,
        {
          method: "DELETE",
          headers: {
      ...
      if (response.ok) {
        setFilteredInventory(
          filteredInventory.filter((item) => item.idinventory !== idinventory)
        );
        setInventory(
          inventory.filter((item) => item.idinventory !== idinventory)
        );
  ...
  useEffect(() => {
    if (activeFilter) {
      setFilteredInventory(
        inventory.filter(
          (item) => item.name_warehouse === activeFilter && item.status_in !== 0
        )
      );
    } else {
  ...
  return (
    <section className="mt-[27px] 930:mt-0 pb-8 930:pb-0 px-[23px] 930:px-0 930:flex-1 930:pr-[24px]">
      {filteredInventory.map((item, index) => {
        ...
        return (
          <div
            ...
            className={`border py-[8px] px-5 930:px-8 rounded-[8px] hover:bg-lightblue cursor-pointer relative
							${index != 0 ? "mt-3" : ""} ${
              isNearExpiration ? "border-red" : "border-second-lightgray"
            }`}
          >
            <a
              onClick={() =>
                check3d(item.idloc, item.idwarehouse_in, item.name_warehouse)
              }
            >
              <div className="flex justify-between">
                ...
                <p className="pt-3 ml-7 930:ml-0">
                  {item.lot_bc && (
                    <span>
                      Batch: <span className="text-red">{item.lot_bc}</span>
                    </span>
                  )}
                  <br></br>
                  <span>
                    Warehouse:{" "}
                    <span className="text-red">{item.name_warehouse}</span>
                  </span>
                  <br></br>
                  <span>
                    Location: <span className="text-red">{item.name_loc}</span>
                  </span>
                  <br></br>
                  <span>
                    Expiry date:{" "}
                    <span className="text-red">
                      {item.batch_ctrl_item == 1 ? formattedExpirationDate : ""}
                    </span>
                  </span>
                </p>
              </div>
            ...
            <button
              ...
              title="Delete user"
              className="hover:opacity-70 absolute top-1 right-2"
            >
              <svg
      ...
      {filteredInventory.length === 0 && (
        <p className="text-start text-red 930:text-[1.2rem] mt-1">
      ...
      {isModalOpen && (
        <DeleteModal
          ...
          itemName={
            selectedInventory!.lot_bc ||
            `${selectedInventory!.name_item} at ${selectedInventory!.name_loc}`
          }
