...
const UserMain = () => {
	...
	const deleteUser = async () => {
		if (selectedUser) {
			const response = await fetch(`${import.meta.env.VITE_REACT_BACKEND_URL}/users/user-delete/${selectedUser.iduser}`, {
				method: 'PUT',
				headers: {
	...
	return (
		<div className="font-manrope flex-1 w-full">
			...
			<div className="flex justify-center 930:justify-start 930:pl-[29px] mt-[27px] mb-[17px] md:mb-[40px] 930:mt-[29px] 930:mb-[21px]">
				<button className="bg-second-lightblue hover:opacity-70 py-[10px] max-w-[222px] w-full font-medium text-dark-color"
					onClick={navToNewUser}>
					<p className='text-center 930:text-start 930:pl-8 '>+ Add new user</p>
				</button>
			</div>
							...
							<tr className="text-[.85rem]" key={item.iduser}>
								...
								<td className="flex items-center justify-center py-[10px] gap-2">
									<button onClick={() => { navToAccessUser(item) }}
										title="User Permissions" className='hover:opacity-70'>
									...
									<button onClick={() => { navToEditUser(item) }}
										title="Update user" className='hover:opacity-70'>
										<svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 18 17" fill="none">
									...
									<button onClick={() => handleOpenModal(item)}
										title="Delete user" className='hover:opacity-70'>
										<svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 18 17" fill="none">
