  ...
  const deleteUser = async () => {
    if (selectedUser) {
      const response = await fetch(
        `${import.meta.env.VITE_REACT_BACKEND_URL}/users/user-delete/${
          selectedUser.iduser
        }`,
        {
          method: "PUT",
      ...
      if (response.ok) {
        ...
        const newUsersList = usersList.filter(
          (item) => item.iduser !== selectedUser.iduser
        );
  ...
  return (
    <div className="font-manrope flex-1 w-full pb-8">
      ...
      <div className="flex justify-center 930:justify-start 930:pl-[29px] mt-[27px] mb-[17px] md:mb-[40px] 930:mt-[29px] 930:mb-[21px]">
        <button
          className="btn-styles relative bg-deep-blue rounded-lg hover:opacity-80 py-[10px] max-w-[222px] w-full font-medium text-dark-color"
          onClick={navToNewUser}
        >
          <p className="text-center 930:text-start 930:pl-8 text-white">Add New Member</p>
        </button>
      </div>
          ...
          <thead>
            <tr className="text-[.85rem]">
              {tableHeading.map((heading, index) => (
                <th
                  key={index}
                  className="border border-second-lightblue text-grayish-gray py-[10px] px-[20px]"
                >
                  {heading}
                </th>
              ))}
