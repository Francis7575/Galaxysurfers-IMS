...
const WarehouseMain = () => {
  ...
  const deleteWarehouse = async () => {
    const response = await fetch(
      `${import.meta.env.VITE_REACT_BACKEND_URL}/warehouses/warehouse-cancel/${
    ...
    if (response.ok) {
      const newUsersList = warehouses.filter(
        (item) => item.idwarehouse !== selectedWarehouse!.idwarehouse
      );
  ...
  return (
    <div className="font-manrope flex-1 w-full pb-8">
      <Heading showBackBtn={false} title="Warehouse Configuration" />
      ...
      <div className="overflow-x-auto px-[20px] 930:px-[29px]">
        <table className="w-full max-w-[600px] mx-auto 930:mx-0">
          ...
          <tbody className="bg-sixth-lightblue cursor-pointer">
            {warehouses.map((item) => (
              <tr
              ...
              >
                ...
                {/* <td className="flex items-center justify-center py-[10px] gap-2">
                  <button onClick={() => navToEditWarehouse(item)}
                    title="Update warehouse" className="hover:opacity-70">
                  ...
                  <button onClick={() => { handleOpenModal(item) }}
                    title="Delete warehouse" className="hover:opacity-70">
                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 18 17" fill="none">
                  ...
                  <button onClick={() => { navToLocations(item) }}
                    title="3D warehouse" className="hover:opacity-70">
                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 18 17" fill="none">
