...
const Threejs: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_REACT_BACKEND_URL}/warehouses/save-locations/${
          warehouse.idwarehouse
        }`,
        {
          method: "POST",
    ...
    <>
      <div className="pb-[10px] 930:flex-1 font-manrope">
        <div className="hidden 930:block">
          <Link to="/warehouseMain" className="w-full">
      ...
      <div className="max-w-[300px] w-full md:max-w-[500px] 930:max-w-none mx-auto 930:mx-0 930:px-[29px]">
        <div className="flex flex-col gap-4 930:flex-row 930:items-center 930:flex-wrap">
          <div className="flex flex-col gap-4">
            <label className="text-black">Width (X)</label>
            <input
              type="number"
              value={newSize[0]}
              onChange={(e) =>
                setNewSize([parseFloat(e.target.value), newSize[1], newSize[2]])
              }
              className="w-full md:w-auto md:min-w-[200px] border border-black rounded-[7px] py-[10px] pl-[10px] outline-none"
            />
          </div>
          <div className="flex flex-col gap-4">
            <label className="text-black">Height (Z)</label>
            <input
              type="number"
              value={newSize[1]}
              onChange={(e) =>
                setNewSize([newSize[0], parseFloat(e.target.value), newSize[2]])
              }
              className="w-full md:w-auto md:min-w-[200px] border border-black rounded-[7px] py-[10px] pl-[10px] outline-none"
            />
          </div>
          <div className="flex flex-col gap-4">
            <label className="text-black">Depth (Y)</label>
            <input
              type="number"
              value={newSize[2]}
              onChange={(e) =>
                setNewSize([newSize[0], newSize[1], parseFloat(e.target.value)])
              }
              className="w-full md:w-auto md:min-w-[200px] border border-black rounded-[7px] py-[10px] pl-[10px] outline-none"
            />
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <button
              ...
              className="w-full hover:opacity-70 bg-blue text-white font-bold py-2 px-4 rounded"
            >
              ADD
            </button>
          </div>
      ...
      <div className="max-w-[300px] w-full md:max-w-[500px] 930:max-w-none mx-auto 930:mx-0 930:px-[29px] mt-3 pb-8">
        <button
          onClick={handleSave}
          className="hover:opacity-70 w-full 930:w-1/6 bg-blue text-white font-bold py-2 px-4 rounded"
        >
          SAVE
        </button>
      </div>
