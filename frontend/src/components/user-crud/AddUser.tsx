...
const AddUser = () => {
	...
	const deleteUser = async (iduser: number) => {
		const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/users/user-delete/${iduser}`, {
			method: 'PUT',
			headers: {
		...
		if (response.ok) {
			...
			const newUsersList = usersList.filter(item => item.iduser !== iduser);
						...
						{usersList.map((item) => (
							<tr className="text-[.85rem]" key={item.iduser}>
								...
								<td className="flex items-center justify-center py-[10px] gap-2">
									...
									<button onClick={() => { deleteUser(item.iduser) }}
													className='hover:opacity-70'>
										<svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 18 17" fill="none">
