...
const ItemMain = () => {
	...
	const tableHeading = ["Code", "Name", "Description", "Unit", "Batch", "Expiration", "Actions"]
	...
	const deleteItem = async (iditem: number) => {
		const response = await fetch(`${import.meta.env.VITE_REACT_BACKEND_URL}/items/item-cancel/${iditem}`, {
			method: 'PUT',
			headers: {
					...
					<tbody className="bg-lightblue ">
						{itemsList.map((item) => (
							<tr className="text-[.85rem]" key={item.iditem}>
								...
								<td className="p-[10px] text-center">{item.name_item}</td>
								<td className="p-[10px] text-center">{item.description_item}</td>
								<td className="p-[10px] text-center">{item.idunit_item}</td>
								<td className="p-[10px] text-center">{item.batch_ctrl_item == 1 ? 'Yes' : 'No'}</td>
								<td className="p-[10px] text-center">{item.expiration_ctrl_item == 1 ? 'Yes' : 'No'}</td>
								<td className="flex items-center justify-center py-[10px] gap-2">
									<button onClick={() => { navToEditItem(item) }} className='hover:opacity-70'>
									...
									<button onClick={() => { handleDeleteClick(item) }} className='hover:opacity-70'>
										<svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 18 17" fill="none">
