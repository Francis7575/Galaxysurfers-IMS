...
const EditItem = () => {
  ...
  const handleSubmit = async (values: NewItem) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_REACT_BACKEND_URL}/items/item-update/${item.iditem}`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <section className="pb-10">
      <a onClick={navigateToItem}><Heading title="Item Configuration" /></a>
